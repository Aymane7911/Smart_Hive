...
// Standalone registration — for customers who already received their SmartHive box.
// Requires serial number from the box sticker.
// On success → account created, device claimed → redirects to /login
// Dashboard access is granted separately by admin.
...
function VerificationModal({
...
}: { email: string; onVerify: (code: string) => Promise<void>; onClose: () => void }) {
  ...
  const handleChange = (i: number, val: string) => {
    ...
    setCode(next);
    setError('');
    if (val && i < 5) setTimeout(() => refs[i + 1].current?.focus(), 0);
    if (i === 5 && val && next.every(d => d)) handleVerify(next.join(''));
  };

  const handleKeyDown = (i: number, e: React.KeyboardEvent) => {
    if (e.key === 'Backspace' && !code[i] && i > 0) {
      const next = [...code]; next[i - 1] = '';
      setCode(next); refs[i - 1].current?.focus();
    }
  };

  const handleVerify = async (fullCode: string) => {
    setLoading(true); setError('');
    try { await onVerify(fullCode); }
    catch (err: any) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 px-4">
      ...
      <div className="relative w-full max-w-sm rounded-2xl p-8 text-center"
        style={{ background: 'rgba(15,15,15,0.92)', border: '1px solid rgba(255,255,255,0.15)', boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }}>
        <div className="w-14 h-14 mx-auto mb-4 rounded-2xl flex items-center justify-center"
          style={{ background: 'linear-gradient(135deg, #f59e0b, #eab308)' }}>
          <Mail className="w-7 h-7 text-white" />
        </div>
        <h2 className="text-xl font-black text-white mb-1">Verify your email</h2>
        ...
        <div className="flex gap-2 justify-center mb-4">
          {code.map((digit, i) => (
            <input
              ...
              disabled={loading}
              className="w-11 text-center text-xl font-bold rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 disabled:opacity-50"
        ...
        {error && (
          <div className="bg-red-500/20 border border-red-400/40 rounded-lg p-3 mb-4">
            <p className="text-red-300 text-xs">{error}</p>
          </div>
        )}

        <button onClick={() => handleVerify(code.join(''))} disabled={loading || code.join('').length < 6}
          className="w-full bg-gradient-to-r from-amber-500 to-yellow-500 text-white py-3.5 rounded-xl font-bold text-sm shadow-lg disabled:opacity-40 disabled:cursor-not-allowed mb-3 transition-all hover:-translate-y-0.5">
          Verify & Activate
        </button>

        <button onClick={handleResend} disabled={timer > 0 || resending}
          className="text-white/40 hover:text-white/70 text-xs disabled:cursor-not-allowed transition-colors">
          {resending ? 'Sending…' : timer > 0 ? `Resend in ${timer}s` : 'Resend code'}
        </button>

        <button onClick={onClose} className="block mx-auto mt-3 text-white/30 hover:text-white/60 text-xs transition-colors">
          Go back
        </button>
      </div>
    </div>
